"""v6 投屏端 — 9:16 抖音竖屏直播深度优化版

核心设计原则:
1. 1080×1080 中心安全区（避开抖音顶栏、底部互动栏、右侧按钮）
2. 字号加大、间距加大（手机小屏可读性优先）
3. 三段式垂直流：汤面 → 谜底揭示 → 互动
4. TTS 朗读字幕条（屏幕底部）
5. 答题气泡（屏幕左侧，向上飘）
6. 全屏礼物特效（高价礼物触发）
7. 适合 OBS 1080×1920 竖屏推流

DPI 适配: OBS 端按 1080×1920 设计，前端用 vh/vw 自适应
抖音手机端真实可见区域: 约 1080×1080（中心安全区）
"""
import json
...
/* ══════════════════════════════════════
   主布局: 1080×1920 竖屏
   抖音安全区 (中心 1080×1080)
   顶部状态 6% + 汤面 13% + 谜底 50% + 进度 5% + 底部 12% = 86%
   (留 14% 给底部互动栏遮挡)
   ══════════════════════════════════════ */
.app{
...
/* ── 顶部状态条 (紧凑型, 5.5vh) ── */
.top-bar{
  flex-shrink:0;height:5.5vh;min-height:38px;
...
.logo{
  width:4.2vh;height:4.2vh;min-width:30px;min-height:30px;
  border-radius:9px;
  background:linear-gradient(135deg,var(--primary),var(--purple));
  display:flex;align-items:center;justify-content:center;
  font-size:2.2vh;flex-shrink:0;
...
.brand-title{font-size:1.7vh;font-weight:900;background:linear-gradient(90deg,#fff,#94a3b8);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.brand-sub{font-size:1.1vh;color:var(--text-dim);letter-spacing:1.5px}

.status-pills{display:flex;align-items:center;gap:1.2vw}
.pill{
  padding:0.5vh 1.6vw;border-radius:99px;
...
/* ── 汤面区 (紧凑, 11-15vh) ── */
.surface-area{
  flex-shrink:0;min-height:11vh;max-height:15vh;
  padding:1.6vh 3.5vw;
  border-radius:14px;
...
.surface-area::after{
  content:'📖';position:absolute;right:2.5vw;top:50%;transform:translateY(-50%);
  font-size:3vh;opacity:0.25;
}
.surface-label{
  font-size:1.2vh;color:var(--primary);
  letter-spacing:3px;margin-bottom:0.6vh;
  display:flex;align-items:center;gap:6px;
  font-weight:700;
}
.surface-label::before{content:'✦';color:var(--gold);font-size:1.4vh}
.surface-text{
  font-size:2.1vh;line-height:1.5;color:var(--text);
...
/* ── 谜底揭示区 (核心, 50vh) ── */
.reveal-area{
  flex:1;min-height:38vh;max-height:55vh;
  border-radius:16px;
...
.reveal-area::before{
  ...
  pointer-events:none;
  animation:reveal-glow 4s ease-in-out infinite;
}
@keyframes reveal-glow{0%,100%{opacity:0.6}50%{opacity:1}}

.reveal-header{
  flex-shrink:0;padding:1.4vh 3vw;
  display:flex;align-items:center;justify-content:space-between;
...
.reveal-scroll{
  flex:1;overflow-y:auto;padding:2.5vh 3vw 1vh;
  display:flex;flex-wrap:wrap;align-content:flex-start;gap:0.8vw;
...
/* ── 字格 (核心视觉, 大字号 + 发光) ── */
.char-box{
  width:6.2vw;height:7vh;min-width:36px;min-height:42px;
  display:flex;align-items:center;justify-content:center;
  font-size:4.4vh;font-weight:900;
...
.char-box.hidden::after{
  content:'';position:absolute;
  width:75%;height:3px;
  background:rgba(148,163,184,0.55);
  border-radius:2px;
}
.char-box.function-word{color:var(--text-dimmer);font-size:3vh;font-weight:400;opacity:0.7}
.char-box.punct{color:var(--text-dimmer);width:3.5vw;font-size:3.2vh}
.char-box.highlight{
  color:var(--gold);
...
/* ── 进度条 ── */
.progress-area{
  flex-shrink:0;padding:1.2vh 3vw;
  border-radius:12px;
  background:linear-gradient(90deg,rgba(12,16,38,0.95),rgba(20,12,40,0.95));
  border:1px solid rgba(0,212,255,0.2);
...
.progress-row{
  ...
  font-size:1.2vh;color:var(--text-dim);margin-bottom:0.6vh;
}
.progress-row .pct{color:var(--primary);font-weight:900;font-size:1.6vh;text-shadow:0 0 10px rgba(0,212,255,0.5)}
.progress-bar{
  height:1.1vh;background:rgba(100,116,139,0.2);
...
/* ── 底部互动区 (紧凑 14vh) ── */
.bottom-area{
  flex-shrink:0;height:14vh;min-height:100px;max-height:18vh;
  display:flex;gap:2vw;
}

.danmaku-panel{
  flex:3;border-radius:14px;
  background:var(--card);border:1px solid var(--card-border);
  backdrop-filter:blur(12px);
  display:flex;flex-direction:column;overflow:hidden;
}
.danmaku-header{
  flex-shrink:0;padding:1vh 2vw;
  display:flex;align-items:center;justify-content:space-between;
  border-bottom:1px solid rgba(255,255,255,0.05);
}
...
.danmaku-scroll{
  flex:1;overflow-y:auto;padding:0.8vh 1.5vw;
...
.dm-tag{
  font-size:1.1vh;padding:0.2vh 1.2vw;border-radius:6px;
...
.info-title{
  font-size:1.2vh;color:var(--text-dim);letter-spacing:2px;
  margin-bottom:0.6vh;flex-shrink:0;
}
.info-stats{
  display:flex;gap:2vw;font-size:1.3vh;
...
.tier-list{
  flex:1;overflow-y:auto;margin-top:0.5vh;
...
.tier-item .tier-badge{
  font-size:1vh;padding:0.1vh 0.8vw;border-radius:4px;
...
.gift-fly .icon-wrap{
  width:20vh;height:20vh;min-width:120px;min-height:120px;
  margin:0 auto 1vh;
...
/* ── 小礼物弹幕 (从右飞入) ── */
.gift-scroll{
  position:fixed;right:2vw;top:8vh;
...
/* ── 答题气泡 (左侧) ── */
.answer-bubbles{
  position:fixed;left:2vw;top:25vh;bottom:30vh;
  z-index:70;pointer-events:none;
  display:flex;flex-direction:column-reverse;gap:1vh;
  max-width:60vw;
}
.answer-bubble{
  display:inline-flex;align-items:center;gap:1.5vw;
...
.tts-bar{
  ...
  z-index:95;
  background:linear-gradient(90deg,rgba(0,0,0,0.85),rgba(12,16,38,0.85));
...
.tts-bar .icon{
  font-size:2vh;color:var(--primary);
...
/* ── 通关结算 ── */
.settlement{
  position:fixed;bottom:14vh;left:4vw;right:4vw;z-index:150;
  background:linear-gradient(180deg,rgba(251,191,36,0.15),rgba(168,85,247,0.15));
  border:1px solid rgba(251,191,36,0.4);
  border-radius:16px;
  padding:2vh 4vw;
  text-align:center;
  animation:settle-up 0.5s ease-out;
...
/* ── 提示横幅 (中下) ── */
.hint-banner{
  position:fixed;left:50%;bottom:32vh;transform:translateX(-50%);
...
.gift-fullscreen .center{
  position:relative;text-align:center;
  animation:center-zoom 2.5s ease-out;
}
...
const CONFIG = {
  ...
  maxDanmaku: 40,           // 弹幕最大保留条数
  giftFullscreenMinCoins: 1000,  // 触发全屏特效的最低抖币
  hintDisappearMs: 60000,   // 提示自动消失时间
  settlementHideMs: 5000,   // 结算弹窗自动消失时间
};
  ...
  switch(msg.type) {
    ...
    case 'game_end':
      ...
      if (msg.bottom) {
        ...
        speakText('谜底揭晓：' + msg.bottom, 12000);
      }
      break;
    case 'tier_up':
      showTierUp(msg);
...
function handleGiftEffect(msg) {
  ...
  if (coins >= CONFIG.giftFullscreenMinCoins) {
    showFullscreenGift(giftName, user, coins);
  } else {
...
function showFullscreenGift(name, user, coins) {
  ...
  div.innerHTML = '<div class="rays"></div><div class="center"><div class="icon">🎉</div><div class="name">' + escapeHtml(name) + '</div><div class="user">感谢 ' + escapeHtml(user) + (coins>0?' · '+coins+'抖币':'') + '</div></div>';
  layer.appendChild(div);
...
function showHint(text, autoHideMs) {
  ...
  banner.innerHTML = '<div class="label">💡 解谜方向</div><div class="text">' + escapeHtml(text) + '</div>';
  layer.appendChild(banner);
  // TTS 朗读
  speakText('提示：' + text, 6000);
  // 自动消失
  setTimeout(() => {
...
// ══════════════════════════════════════════
// TTS 字幕 (使用浏览器 Web Speech API)
// ══════════════════════════════════════════
let ttsTimeout = null;
function speakText(text, hideMs) {
  ...
  ttsTimeout = setTimeout(() => bar.classList.remove('active'), hideMs || 6000);
}
...
function renderTierList(list) {
  ...
  container.innerHTML = list.map(t =>
    ...
    (t.tier ? '<span class="tier-badge">' + escapeHtml(t.tier) + '</span>' : '') +
